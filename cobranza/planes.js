...
// En la pantalla operativa, registra la oferta en Supabase antes de abrir WhatsApp.
// Si la bitácora falla, no se bloquea el trabajo manual de Rosaura.
window.addEventListener('load',()=>{
 if(typeof window.sendOffer!=='function'||typeof window.rpc!=='function')return;
 const abrir=window.sendOffer;
 window.sendOffer=async function(id,term){
  try{
   const r=(window.residentes||[]).find(x=>Number(x.id_residente)===Number(id));
   if(r&&typeof window.calc==='function'){
    const p=window.calc(r).find(x=>x.meses===Number(term));
    if(p){
     await window.rpc('pg_registrar_oferta_regularizacion',{...window.C,p_id_residente:r.id_residente,p_plan:p.nombre,p_plazo_meses:p.meses,p_meses_vencidos:typeof window.meses==='function'?window.meses(r):0,p_saldo_historico:typeof window.hist==='function'?window.hist(r):p.adeudoOriginal,p_descuento_pct:p.descuentoPct,p_saldo_regularizado:p.adeudoConDescuento,p_cuota_corriente:p.cuotaCorriente,p_abono_deuda:p.abonoDeuda,p_total_mensual:p.totalMensual,p_total_quincenal:p.totalQuincenal,p_estado:'ofrecida',p_notas:'Oferta preparada desde módulo de regularización'});
    }
   }
